...
export default function AdminHeader({ onToggleSidebar }) {
  ...
  const breadcrumbs = pathParts.map((part, index) => {
    ...
    if (part === 'profile') label = 'Executive Profile';
    ...
    <header className="sticky top-0 z-30 w-full bg-white/95 backdrop-blur-md border-b border-slate-200/80 px-4 sm:px-8 py-3.5 flex items-center justify-between transition-all shadow-xs">
      ...
      <div className="flex items-center gap-2.5 sm:gap-3.5">
        ...
        <div className="relative">
          ...
          >
            <div className="w-7 h-7 rounded-lg bg-emerald-600 text-white flex items-center justify-center font-bold text-xs shadow-xs overflow-hidden shrink-0">
              {admin?.avatar ? (
                <img
                  src={admin.avatar}
          ...
          {profileDropdownOpen && (
            <div className="absolute right-0 mt-2 w-60 bg-white border border-slate-200 rounded-2xl p-1.5 shadow-xl z-50 animate-in fade-in slide-in-from-top-2 duration-150">
              ...
              <div className="p-1 space-y-0.5">
                <Link
                  ...
                  className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-xs font-semibold text-emerald-800 bg-emerald-50/70 hover:bg-emerald-100/70 transition-colors"
                >
                  <User className="w-4 h-4 text-emerald-600" />
                  <div className="flex flex-col">
                    <span>Edit My Profile</span>
                    <span className="text-[10px] font-normal text-emerald-600">Update details & password</span>
                  </div>
                </Link>
                <Link
